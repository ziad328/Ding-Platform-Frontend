/**
 * noroChat utils — time-grouping helpers for sidebar session lists
 * and title/id generation utilities.
 */
...
export function groupConversationsByTime(
  ...
  const map: Record<TimeGroup, NoroChatConversation[]> = { Today: [], 'Previous 7 Days': [], 'Previous 30 Days': [], Older: [] };
  for (const conv of conversations) map[getTimeGroup(conv.createdAt)].push(conv);
...
export function groupSessionsByTime(
  ...
  const order: TimeGroup[] = ['Today', 'Previous 7 Days', 'Previous 30 Days', 'Older'];
  const map: Record<TimeGroup, ChatSession[]> = { Today: [], 'Previous 7 Days': [], 'Previous 30 Days': [], Older: [] };
  for (const session of sessions) map[getTimeGroup(new Date(session.created_at))].push(session);
  return order.filter((g) => map[g].length > 0).map((g) => ({ group: g, items: map[g] }));
}
