...
const ProfileHoverPreview = ({
  ...
  return createPortal(
    ...
    >
      <div className="relative rounded-2xl border border-neutral-w-300 bg-white text-neutral-b-900 p-4 shadow-xl">
        <div
          className={`absolute h-4 w-4 rotate-45 bg-white border-l border-t border-neutral-w-300 ${
            placement.position === 'top' ? 'bottom-0 translate-y-1/2' : '-top-2'
          }`}
          style={{ left: placement.arrowOffset }}
        />
        <div className="flex items-start gap-3">
          <div className="w-14 h-14 rounded-full overflow-hidden border-2 border-neutral-w-300">
          ...
          <div>
            ...
            <p className="text-sm text-neutral-b-600 leading-snug">
              {friend.role}
              {friend.company ? ` · ${friend.company}` : ''}
            </p>
            {friend.location && (
              <p className="text-xs text-neutral-b-500 mt-1">{friend.location}</p>
            )}
          </div>
        </div>
        {friend.bio && <p className="text-sm text-neutral-b-700 mt-3">{friend.bio}</p>}
        <div className="mt-3 space-y-1 text-xs text-neutral-b-600">
          {friend.company && (
          ...
          {friend.focus && <p className="text-neutral-b-500">Focus: {friend.focus}</p>}
        </div>
